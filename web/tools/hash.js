...
// MD5 for ArrayBuffer (handles binary data)
...
export function run(Tool) {
  ...
  const input = Tool.textarea('输入要计算哈希的文本...');
  ...
  row.appendChild(Tool.btn('计算', doHash, 'primary'));
  ...
  const output = Tool.textarea('哈希结果', true);
  ...
  async function doHash() {
    ...
    try {
      ...
      if (alg === 'MD5') {
        ...
        result = bufToHex(hashBuf);
      }
      output.value = upper ? result.toUpperCase() : result;
    } catch (e) {
      Tool.showErr(errEl, '计算失败: ' + e.message);
    }
  }
}
